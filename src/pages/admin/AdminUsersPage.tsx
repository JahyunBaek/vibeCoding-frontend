...
export default function AdminUsersPage() {
  ...
  const filtered = allItems.filter((u) => {
    ...
    return (
      u.username?.toLowerCase().includes(q) || u.name?.toLowerCase().includes(q) || u.roleKey?.toLowerCase().includes(q)
    );
  });
  ...
  const saveMut = useMutation({
    mutationFn: () =>
      api.userUpdate(editUser.userId, {
        name: editName,
        roleKey: editRoleKey,
  ...
  return (
    <div className="space-y-4">
        ...
        {showCreate && (
          ...
          >
            ...
            <div className="grid gap-2 md:grid-cols-2">
              ...
              <select className="h-9 rounded-md border bg-surface px-3 text-sm" {...registerCreate("roleKey")}>
              ...
              <select className="h-9 rounded-md border bg-surface px-3 text-sm" {...registerCreate("orgId")}>
        ...
        {resetUser && (
          <div className="mx-6 mb-4 rounded-lg border border-amber-500/30 bg-amber-500/10 p-4 space-y-3">
            <div className="text-xs font-medium text-amber-600 uppercase tracking-wide">
              {t("admin.passwordReset")} — {resetUser.username}{" "}
              <span className="text-amber-400">(ID: {resetUser.userId})</span>
            </div>
              ...
              <Button
                onClick={() => {
                  if (resetPassword.length < 8) {
                    setResetError(t("admin.passwordResetMinLength"));
                    return;
                  }
                  resetMut.mutate();
                }}
      ...
      {resetLinkUser && (
        ...
        >
          <div
            className="w-full max-w-lg rounded-lg border bg-surface p-6 shadow-xl"
          ...
          >
            ...
            {resetTokenMut.isPending ? (
              ...
              <div className="mt-4 space-y-3">
                ...
                <p className="text-xs text-muted-fg">{t("admin.linkExpiry", { minutes: resetLinkExpiry })}</p>
              </div>
            ) : null}

            <div className="mt-5 flex justify-end">
              <Button variant="outline" onClick={() => setResetLinkUser(null)}>
                {t("common.close")}
              </Button>
            </div>
          </div>
      ...
      {showInvite && (
        ...
        >
          ...
          >
            ...
            <p className="mt-1 text-sm text-muted-fg">{t("admin.inviteDescription")}</p>
            ...
            {!inviteLink ? (
              ...
              <div className="mt-4 space-y-3">
                ...
                <p className="text-xs text-muted-fg">{t("admin.inviteLinkGuide")}</p>
